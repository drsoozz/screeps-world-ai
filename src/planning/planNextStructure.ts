...
import { getVeniceRoadCoords } from "./structures/getVeniceRoadCoords";
import { hasStructureOrSite } from "utils/hasStructureOrConstructionSite";
import { DehydratedRoomPosition, getRehydratedRoomPosition } from "types/DehydratedRoomPosition";
...
import { structurePlanningDataRoads } from "types/memory";
...
function _planRoad(
  room: Room,
  roadPlan: structurePlanningDataRoads,
  generator: (room: Room) => DehydratedRoomPosition[],
 ...
 */
function _planVeniceRoad(room: Room, amOwner: boolean) {
  ...
  const generatedMessage = `Road data for structure planning was generated for ${room.name}.`;

  return _planRoad(room, roadPlan, getVeniceRoadCoords, generatedMessage);
}
...
function _planExploitationRoads(room: Room, amOwner: boolean) {
  ...
  const generatedMessage = `Exploitation road data for structure planning was generated for ${room.name}.`;
  return _planRoad(room, roadPlan, getExploitationPlanCoords, generatedMessage);
}
